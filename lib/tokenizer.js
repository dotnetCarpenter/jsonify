...
function tokenize(str) {
	...
	if( isAssign(c) ) return prepend(new TokAssign, tokenize(cs))
	if( isLParen(c) ) return prepend(new TokLParen, tokenize(cs))
	if( isRParen(c) ) return prepend(new TokRParen, tokenize(cs))
...
console.log("a + (b - 22) = y/2 ")
console.dir(tokenize("a + (b - 22) = y/2 "))
