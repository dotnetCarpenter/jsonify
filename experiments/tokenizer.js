...
function tokenize(str) {
	...
	if( isAssign(c) ) return prepend("=", tokenize(cs))
	if( isLParen(c) ) return prepend("(", tokenize(cs))
	if( isRParen(c) ) return prepend(")", tokenize(cs))
...
console.log("(a + b - 22 = y/2)", tokenize("(a + b - 22 = y/2)") )
